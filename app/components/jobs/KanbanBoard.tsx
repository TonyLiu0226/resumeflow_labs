...
export default function KanbanBoard({ resumes, onNavigateToKeywords }: KanbanBoardProps) {
  ...
  const [expandedSection, setExpandedSection] = useState<JobStatus | null>(JOB_STATUSES[0]);
  ...
  async function updateJobStatus(jobId: string, newStatus: JobStatus) {
    ...
    try {
      ...
      if (selectedJob?.id === jobId) {
        setSelectedJob((prev) => (prev ? { ...prev, status: job.status } : null));
      }
    }
  }
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className={`px-4 py-3 w-full flex items-center justify-between transition-colors ${
                    isExpanded ? `rounded-t-xl ${config.headerBg}` : "rounded-xl bg-zinc-50 hover:bg-zinc-100"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <h3 className={`text-base font-semibold ${isExpanded ? config.color : "text-zinc-700"}`}>
                      {config.label}
                    </h3>
                    <span
                      className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-bold ${
                        isExpanded ? `${config.color} ${config.bgColor}` : "bg-zinc-200 text-zinc-600"
                      }`}
                    >
                      {columnJobs.length}
                    </span>
                ...
                {/* Cards / Content */}
                {isExpanded && (
                  <div className={`p-4 bg-zinc-50/50 rounded-b-xl border-t ${isDragOver ? config.borderColor : 'border-zinc-100'} min-h-[150px]`}>
                    {columnJobs.length > 0 ? (
                      ...
                      <div className="flex items-center justify-center h-full min-h-[100px] text-sm text-zinc-400">
                        No jobs in this category. Drop jobs here.
                      </div>
                    )}
                  </div>
                )}
